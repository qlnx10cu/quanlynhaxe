...
module.exports = {
    ...
    exportBill: async function(req, res, next) {
        ...
        chitiet['tongtienpt'] = ws_data.chitiet.reduce((prev, cur) => prev += cur.tinhtien, 0);
        ...
        for(var i=0;i<chitiet.length;i++){
                chitiet[i].tinhtien=chitiet[i].tinhtien.toLocaleString('vi-VI', { style: 'currency', currency: 'VND' })
                chitiet[i].dongia= chitiet[i].dongia.toLocaleString('vi-VI', { style: 'currency', currency: 'VND' })
                chitiet[i].tiencong= chitiet[i].tiencong.toLocaleString('vi-VI', { style: 'currency', currency: 'VND' })
                chitiet[i].tongtien= chitiet[i].tongtien.toLocaleString('vi-VI', { style: 'currency', currency: 'VND' })
        }
