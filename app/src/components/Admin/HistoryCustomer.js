...
const HistoryCustomer = (props) => {
    ...
    useEffect(() => {
        setData({});
        if (ma) {
            GetCustomerDetail(props.token, ma)
                .then((res) => {
                    setData(res.data);
                })
                .catch((err) => {
    ...
    return (
        <Modal className={props.isShowing ? "active" : ""}>
            <ModalContent>
                ...
                {activePage === 0 && <RenderTableDetail list={mData.chitiet} {...props} />}
                {activePage === 1 && <RenderTableCSKH list={mData.historycskh} {...props} />}
                {activePage === 2 && <RenderTableCall list={mData.historycall} {...props} />}
            </ModalContent>
        </Modal>
    );
};
