...
const BanLe = (props) => {
    ...
    const handleSaveBill = () => {
        ...
        UpdateBillBanLe(props.token, data)
            .then((res) => {
    ...
    const handleAddBill = () => {
        ...
        if (makhachhang) data.makh = makhachhang;

        SaveBillBanLe(props.token, data)
            .then((res) => {
                clearAll();
    ...
    const deleteProduct = (it) => {
        ...
        if (it.maphutung && it.maphutung !== "") {
            setProducts(
                mProducts.filter(function (item) {
    ...
    const handleChangeKH = (value, enter) => {
        ...
        if (kq) {
            ...
            var resCheck = 0;
            GetlistCustomer(props.token, `ma=${value}`)
                .then((res) => {
                    try {
                    ...
                    if (resCheck != 0) props.addLoading();
                    else resCheck = 1;
                })
                .catch((err) => {
                    if (resCheck != 0) props.addLoading();
    ...
    const handleChangeSDT = (value, enter) => {
        ...
        if (kq) {
            ...
            var resCheck = 0;
            GetlistCustomer(props.token, `sodienthoai=${value}`)
                .then((res) => {
                    try {
                        if (res.data && res.data[0]) {
                            mCustomerName.setValue(res.data[0].ten);
                                ...
                                <tr key={index}>
                                    ...
                                    <td>
                                        <input
                                            ...
                                            value={mProducts[index].chietkhau}
                                            min="0"
                                        />
                                    </td>
