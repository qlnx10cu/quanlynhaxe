...
const PopupCuaHangNgoai = (props) => {
    ...
    const handleLuu = () => {
        if (tenphutung === "" || nhacungcap === "" || !dongia || dongia === 0) {
            ...
            if (!props.item) {
                SaveItemCuaHangNgoai(props.token, data)
                    .then((res) => {
                        clearData();
                        props.getList();
                        props.onCloseClick();
                ...
                //    Update
                UpdateItemCuaHangNgoai(props.token, data, props.item.tenphutung, props.item.nhacungcap)
                    .then((res) => {
                        clearData();
                        props.getList();
                        props.onCloseClick();
        ...
        <Modal className={props.isShowing ? "active" : ""}>
            <ModalContent>
                ...
                <DivFlexRow style={{ width: "100%", justifyContent: "flex-end" }}>
                    <Button onClick={handleLuu}>Lưu</Button>
