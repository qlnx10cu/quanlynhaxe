...
const ConfirmWarrper = (props) => {
    ...
    return (
        <ModalAlert className={props.confirm.isLoading ? "active" : ""}>
            <ModalContent
                style={{ width: "700px", backgroundColor: props.confirm.error == 0 ? "#fefefe" : props.confirm.error == 1 ? "#fcff40fc" : "#ff4100" }}
            >
                ...
                <DivFlexRow style={{ marginTop: 50, marginBottom: 5, justifyContent: "flex-end", alignItems: "center" }}>
                    ...
                    <CancleButton onClick={() => onCloseClick()}>Hủy </CancleButton>
                    <DivFlexRow></DivFlexRow>
                    <Button onClick={() => handleCallback(props.confirm.callback)}>Đồng ý </Button>
                </DivFlexRow>
            </ModalContent>
        </ModalAlert>
    );
...
const mapDispatch = (dispatch) => ({
    setConfirm: (isLoad, mess, callback) => {
        dispatch(setConfirm(isLoad, mess, callback));
    },
});
